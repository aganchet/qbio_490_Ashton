...
```{r}
write.csv(clinic, "/home1/aganchet/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/aganchet/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
The p value is 0.2 meaning this classification of data is not statistically significant and we cant confidently state a correlation between age and breast cancer survival based on TCGA.